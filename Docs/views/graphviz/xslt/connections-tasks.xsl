...
<xsl:strip-space elements="host_list"/>
  ...
  <xsl:if test="action">
    ...
    <xsl:text> [ color="#000000ff", fontcolor = "white", labelfontsize = 10, taillabel = "</xsl:text>
          <xsl:call-template name="no_quotes">
	...
	-->
	  ...
	  <xsl:value-of select="$nice_shortname"/>
	  <xsl:text> -> sonettnet [ color = "blue", fontcolor="red", labelangle = 45, labeljust = "l" , labelloc="t", fontsize=10, taillabel = "BLAH" ];
</xsl:text>
	</xsl:if>
